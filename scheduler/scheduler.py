...
def run_scheduler(model, scaler, num_tasks=30):
    ...
    for i in range(num_tasks):
        ...
        if feasible:
            ...
            if decision == 0:
                ...
                metrics["fog"] += 1
                print(f"ML Scheduler Decision: FOG -> Assigned to {node_id}")
            else:
                node_id = allocate_node(cloud_nodes, task_load)
                current_cloud_load += task_load
                metrics["cloud"] += 1
                print(f"ML Scheduler Decision: CLOUD -> Assigned to {node_id}")
    ...
    # ✅ RETURN REAL METRICS (USED BY VISUALIZATION)
    return {
